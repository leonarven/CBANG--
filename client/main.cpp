...
const unsigned short PORT        = 6665; // The port we are connecting to
...
int main(int argc, char **argv)
{
    ...
    else // If we successfully opened a connection then check for the server response to our connection
    {
        ...
        if (gotServerResponse != 0) {
            ...
            if ( buffer[0] == 'o' ) {
            } else cout << "Got the following from server: " << buffer << "(" << serverResponseByteCount << " bytes)" << endl;
